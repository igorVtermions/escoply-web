...
export default async function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const { data: profile } = await supabase.from("profiles").select("full_name, company_name, avatar_path").eq("id", user.id).maybeSingle<{ full_name: string; company_name: string | null; avatar_path: string | null }>();
