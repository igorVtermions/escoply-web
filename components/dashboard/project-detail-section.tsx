...
export function ProjectDetailSection({ project }: { project: ProjectDetailData }) {
  ...
  function exportSummary() {
    ...
    const payments = project.payments.map((payment) => `<tr><td>${escapeHtml(payment.description)}</td><td>${formatShortDate(payment.dueDate)}</td><td>${currencyFormatter.format(payment.amount)}</td><td>${escapeHtml(paymentLabels[payment.status] ?? payment.status)}</td></tr>`).join("");
        ...
        <main className="project-detail-main">
          ...
          <SectionCard number={6} title="Pagamentos / Recebimentos" icon={DollarSign} action={<button type="button" onClick={() => setModal("payment")}><Plus size={14} /> Recebimento</button>}>
            <div className="project-detail-payments">
              <div><span>Descrição</span><span>Data</span><span>Valor</span><span>Status</span><span>Comprovante</span><span>Recebido em</span><span>Ação</span></div>
      ...
      {modal === "payment" && (
        <ActionModal title="Novo recebimento" eyebrow="Financeiro" onClose={() => setModal(null)}>
          <form className="project-detail-form" onSubmit={submitWith(createPaymentAction, "Recebimento criado")}>
            ...
            <div className="project-detail-form-grid"><label>Valor<CurrencyInput name="amount" required /></label><label>Data do recebimento<input name="due_date" type="date" required /></label></div>
