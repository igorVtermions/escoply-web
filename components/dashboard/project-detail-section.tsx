...
const approvalLabels: Record<string, string> = { pending: "Pendente", approved: "Aprovado", rejected: "Recusado", review: "Em revisão" };
...
const approvalSuggestions = [
  { title: "Briefing aprovado", note: "Cliente confirmou que o briefing e objetivos estão corretos." },
  { title: "Escopo aprovado", note: "Cliente validou o que entra e o que não entra neste projeto." },
  { title: "Orçamento aprovado", note: "Cliente aceitou valor, prazo e condição de pagamento." },
  { title: "Aprovação final", note: "Cliente validou a entrega final do projeto." },
];
const materialTabLabels: Record<"file" | "link" | "note", string> = { file: "Arquivos", link: "Links", note: "Anotações" };

type ModalName = "edit" | "reminder" | "budget" | "scope" | "approval" | "material" | "payment" | "reminders" | null;
...
export function ProjectDetailSection({ project }: { project: ProjectDetailData }) {
  ...
  function createSuggestedApproval(suggestion: { title: string; note: string }) {
    const formData = new FormData();
    formData.set("project_id", project.id);
    formData.set("title", suggestion.title);
    formData.set("note", suggestion.note);
    runAction(() => createApprovalAction(initialState, formData), "Aprovação criada");
  }
  ...
  function exportSummary() {
    const lines = [
      ...
      `Valor estimado: ${currencyFormatter.format(project.estimatedValue)}`,
      "",
      "Resumo:",
      project.description || "Sem resumo cadastrado.",
      "",
      "Escopo:",
      ...project.scopeItems.map((item) => `- ${item.completedAt ? "[x]" : "[ ]"} ${item.title}`),
      "",
      "Recebimentos:",
      ...project.payments.map((payment) => `- ${payment.description}: ${currencyFormatter.format(payment.amount)} (${paymentLabels[payment.status] ?? payment.status})`),
    ];
    const blob = new Blob([lines.join("\n")], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `resumo-${project.name.toLowerCase().replace(/[^\w]+/g, "-")}.txt`;
    link.click();
  ...
  return (
    <div className="project-detail-page">
      ...
      <div className="project-detail-layout">
        <main className="project-detail-main">
          ...
          <div className="project-detail-grid middle">
            <SectionCard number={4} title="Aprovações" icon={CheckCircle2} action={<button type="button" onClick={() => setModal("approval")}><Plus size={14} /> Aprovação</button>}>
              <div className="project-detail-timeline">
                {project.approvals.length === 0 && (
                  <div className="project-detail-approval-empty">
                    <strong>Nenhuma aprovação cadastrada ainda.</strong>
                    <p>Use aprovações para registrar validações do cliente e evitar retrabalho depois.</p>
                    <div>
                      {approvalSuggestions.map((suggestion) => (
                        <button type="button" key={suggestion.title} disabled={isPending} onClick={() => createSuggestedApproval(suggestion)}>
                          <Plus size={13} /> {suggestion.title}
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {project.approvals.map((approval) => (
                  <div key={approval.id} className={approval.status === "approved" ? "done" : approval.status === "review" ? "current" : ""}>
                    <strong>{approval.title}</strong><span>{approval.note || approvalLabels[approval.status] || approval.status}</span>
                    <div className="project-detail-inline-actions">
                      <button type="button" onClick={() => runAction(() => updateApprovalStatusAction(approval.id, project.id, "approved"), "Aprovação atualizada")}>Aprovar</button>
                      <button type="button" onClick={() => runAction(() => updateApprovalStatusAction(approval.id, project.id, "review"), "Aprovação atualizada")}>Revisão</button>
                      <button type="button" onClick={() => runAction(() => updateApprovalStatusAction(approval.id, project.id, "rejected"), "Aprovação atualizada")}>Recusar</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <SectionCard number={5} title="Materiais" icon={FolderOpen} action={<button type="button" onClick={() => setModal("material")}><Plus size={14} /> Material</button>}>
              ...
              {filteredMaterials.length === 0 ? (
                ...
                <div className="project-detail-material-list">
                  {filteredMaterials.map((material) => (
                    <div key={material.id}>
                      <FileText size={17} />
                      <span><strong>{material.title}</strong><small>{material.kind === "file" ? `${material.mimeType ?? "Arquivo"} · ${material.fileSize ? `${Math.round(material.fileSize / 1024)} KB` : ""}` : material.kind === "link" ? material.url || "Link salvo" : material.note || "Anotação"}</small></span>
                      {(material.fileUrl || material.url) && <a href={material.fileUrl || material.url || "#"} target="_blank" rel="noreferrer"><ExternalLink size={16} /></a>}
            ...
            <SectionCard number={6} title="Lembretes do projeto" icon={Bell} action={<button type="button" onClick={() => setModal("reminder")}><Plus size={14} /> Novo</button>}>
          ...
          <SectionCard number={7} title="Pagamentos / Recebimentos" icon={DollarSign} action={<button type="button" onClick={() => setModal("payment")}><Plus size={14} /> Recebimento</button>}>
            <div className="project-detail-payments">
              <div><span>Descrição</span><span>Vencimento</span><span>Valor</span><span>Status</span><span>Tipo</span><span>Recebido em</span><span>Ação</span></div>
              {project.payments.length === 0 && <p>Nenhum pagamento cadastrado para este projeto.</p>}
              {project.payments.map((payment) => <div key={payment.id}><strong>{payment.description}</strong><time>{formatShortDate(payment.dueDate)}</time><span>{currencyFormatter.format(payment.amount)}</span><em className={payment.status}>{paymentLabels[payment.status] ?? payment.status}</em><span>Recebimento</span><time>{payment.paidAt ? shortDateFormatter.format(new Date(payment.paidAt)) : "—"}</time><button type="button" disabled={payment.status === "paid"} onClick={() => runAction(() => markPaymentPaidAction(payment.id, project.id), "Recebimento atualizado")}>Marcar pago</button></div>)}
      ...
      {modal === "edit" && (
        <ActionModal title="Editar projeto" eyebrow="Projeto" onClose={() => setModal(null)}>
          <form className="project-detail-form" onSubmit={handleEditProject}>
            ...
            <div className="project-detail-form-grid"><label>Prazo<input name="deadline" type="date" defaultValue={project.deadline ?? ""} /></label><label>Valor estimado<input name="estimated_value" defaultValue={toCurrencyInput(project.estimatedValue)} /></label></div>
      ...
      {modal === "budget" && (
        <ActionModal title="Editar orçamento" eyebrow="Financeiro" onClose={() => setModal(null)}>
          <form className="project-detail-form" onSubmit={submitWith(saveBudgetAction, "Orçamento salvo")}>
            ...
            <label>Valor<input name="amount" defaultValue={toCurrencyInput(budget?.amount ?? project.estimatedValue)} required /></label>
      ...
      {modal === "approval" && (
        <ActionModal title="Nova aprovação" eyebrow="Validação" onClose={() => setModal(null)}>
          <form className="project-detail-form" onSubmit={submitWith(createApprovalAction, "Aprovação criada")}>
            <input type="hidden" name="project_id" value={project.id} />
            <label>Título<input name="title" placeholder="Ex.: Wireframe aprovado" required minLength={2} /></label>
            <label>Observação<textarea name="note" rows={3} placeholder="Contexto da aprovação" /></label>
            <button type="submit" disabled={isPending}>{isPending ? "Criando..." : "Criar aprovação"}</button>
          </form>
        </ActionModal>
      )}
      ...
      {modal === "material" && (
        <ActionModal title="Adicionar material" eyebrow="Materiais" onClose={() => setModal(null)}>
          <form className="project-detail-form" onSubmit={submitWith(createMaterialAction, "Material salvo")}>
            <input type="hidden" name="project_id" value={project.id} />
            <label>Tipo<select name="kind" defaultValue={activeMaterialTab}><option value="file">Arquivo</option><option value="link">Link</option><option value="note">Anotação</option></select></label>
            ...
            <label>Arquivo<input name="file" type="file" accept="application/pdf,image/png,image/jpeg,image/webp,text/plain" /></label>
            <label>Anotação<textarea name="note" rows={3} /></label>
      ...
      {modal === "payment" && (
        <ActionModal title="Novo recebimento" eyebrow="Financeiro" onClose={() => setModal(null)}>
          <form className="project-detail-form" onSubmit={submitWith(createPaymentAction, "Recebimento criado")}>
            ...
            <div className="project-detail-form-grid"><label>Valor<input name="amount" required /></label><label>Vencimento<input name="due_date" type="date" required /></label></div>
